...
class IndentChecker(BaseTokenChecker):
    ...
    msgs = {
        'W0313':
        ('File contains mixed indentation - some lines use tabs, some lines use spaces',
         'indentation-mixture',
         'Used when there are some mixed tabs and spaces in a module.'),
        'W0314':
        ('Line uses %s for indentation, but %s required',
         'incorrect-indentation',
         'Used when the indentation of a line does not match the style required by configuration.'
         )
    }
    options = (('indent-strict-spaces', {
        'default': False,
        'type': "yn",
        'metavar': '<boolean>',
            ...
            if token[0] == tokenize.NEWLINE:
                ...
                if line.startswith('\t'):
                    if self.config.indent_strict_spaces:
                        ...
                        self.add_message('incorrect-indentation',
                                         line=line_num,
                                         args=('tabs', 'spaces'))
                    tab_count += 1
